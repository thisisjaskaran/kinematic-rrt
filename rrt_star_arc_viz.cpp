...
public:
    ...
    void display_arc(struct Node* node_1, struct Node* node_2)
    {
        int x1 = node_1->x;
        int y1 = node_1->y;
        int x2 = node_2->x;
        ...
        if(theta_2 > theta_1)
        {
            for(double theta = theta_1; theta < theta_2; theta += 0.01)
            {
                int x = 0, y = 0;
                
                x = round(xc - rho * sin(theta));
                y = round(yc + rho * cos(theta));
                ...
                world.at<cv::Vec3b>(x,y)[0] = 128;
                world.at<cv::Vec3b>(x,y)[1] = 128;
                world.at<cv::Vec3b>(x,y)[2] = 128;
            }
        }
        else
        {
